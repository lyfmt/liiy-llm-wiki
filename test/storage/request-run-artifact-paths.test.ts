...
describe('buildRequestRunArtifactPaths', () => {
  it('builds the spec-required artifact paths under state/runs/<run_id>', () => {
    ...
    expect(buildRequestRunArtifactPaths(root, runId)).toEqual({
      ...
      toolOutcomes: path.join(root, 'state', 'runs', runId, 'tool-outcomes.json'),
      events: path.join(root, 'state', 'runs', runId, 'events.json'),
      timeline: path.join(root, 'state', 'runs', runId, 'timeline.json'),
