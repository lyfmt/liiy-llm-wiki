...
`,
  ...
  it('rejects input artifacts and output directories outside state/artifacts/subagents', async () => {
    const root = await mkdtemp(path.join(tmpdir(), 'llm-wiki-runtime-run-subagent-'));
    const faux = registerFauxProvider({
      api: 'test-runtime-subagent',
      provider: 'test-runtime-subagent',
...
`,
      ...
      await expect(
        tool.execute('tool-call-1', {
          profile: 'worker',
          taskPrompt: 'Read the provided artifact and write a receipt.',
          inputArtifacts: ['state/artifacts/chat-settings.json'],
          outputDir: 'state/artifacts/subagents/run-001--subagent-guard-1'
        })
      ).rejects.toThrow('Subagent input artifacts must stay within state/artifacts/subagents/');
