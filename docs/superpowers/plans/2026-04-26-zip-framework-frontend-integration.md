...
- [x] 步骤 1：运行 `npm run typecheck:web` 记录当前基线。
- [x] 步骤 2：移除全局像素字体和 `.pixel-*` 依赖，把 body 调整为清爽 sans 字体、浅蓝白背景、普通字距。
- [x] 步骤 3：创建可复用的顶部导航/悬浮聊天按钮，使用 `/app`、`/app/kb`、`/app/raw`、`/app/console`、`/app/ai-chat` 路径。
- [x] 步骤 4：运行 `npm run typecheck:web`。
- [x] 步骤 5：Commit：`style(web): 接入 zip 风格视觉基础`。
...
- [x] 步骤 1：用真实 `useDiscovery()` 数据编写 Recent Updates 行为验证思路：只显示非 source 项。
- [x] 步骤 2：把 zip `Home.tsx` 的艺术化 hero、简洁顶栏和 recent list 移植到当前页面。
- [x] 步骤 3：保持 Raw 顶栏入口和 `/app/kb` 主按钮。
- [x] 步骤 4：运行 `npm run typecheck:web && npm run build:web`。
- [x] 步骤 5：Commit：`feat(web): 使用 zip 框架改造首页`。
...
- [x] 步骤 1：保留现有 `useKnowledgeNavigation()` 和 drill-down 状态。
- [x] 步骤 2：将左侧栏和内容卡片调整为 zip 的简洁白底、slate 文本、brand 蓝交互。
- [x] 步骤 3：确保页面仍是 `h-screen overflow-hidden`，左右栏内部滚动。
- [x] 步骤 4：运行 `npm run typecheck:web && npm run build:web`。
- [x] 步骤 5：Commit：`feat(web): 使用 zip 框架改造知识库浏览`。
...
- [x] 步骤 1：`/app/raw` 使用真实 `useRawSources()` 列表，默认选中第一项并调用 `useRawSource()` 展示只读预览。
- [x] 步骤 2：保留 `/app/raw/:sourceId` 深链接阅读页，使用 zip 的行号/片段高亮风格。
- [x] 步骤 3：运行 `npm run typecheck:web && npm run build:web`。
- [x] 步骤 4：Commit：`feat(web): 使用 zip 框架改造 Raw 资源页`。
...
- [x] 步骤 1：保留 `getKnowledgePage()`、Markdown 渲染、source refs、related_by_source。
- [x] 步骤 2：统一为 zip 的阅读排版和简洁侧栏。
- [x] 步骤 3：运行 `npm run typecheck:web && npm run build:web`。
- [x] 步骤 4：Commit：`feat(web): 使用 zip 框架改造阅读页`。
...
- [x] 步骤 1：新增 `getKnowledgeInsertPipeline(runId)` API 与最小 Pipeline 状态类型。
- [x] 步骤 2：上传附件收到 `pipeline_run_id` 后记录到页面状态。
- [x] 步骤 3：轮询 `/api/knowledge-insert/pipelines/:id`，404 时显示“启动中/等待状态文件”，成功后显示当前阶段/状态。
- [x] 步骤 4：在 Chat 页面附件区或侧栏显示 Knowledge Insert 状态。
- [x] 步骤 5：运行 `npm run typecheck:web && npm run build:web`。
- [x] 步骤 6：Commit：`feat(web): 显示 Knowledge Insert 流水线状态`。

## 补充任务：Chat 与 Settings 视觉收口

**文件：**
- 修改：`web/src/features/ai-chat/pages/ai-chat-page.tsx`
- 修改：`web/src/features/console/pages/console-page.tsx`

- [x] 步骤 1：移除 Chat 页残留 `.pixel-*`、厚重黑边、命令台大写文本与像素风阴影。
